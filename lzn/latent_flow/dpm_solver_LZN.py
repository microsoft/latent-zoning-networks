...
class NoiseScheduleRectFlow:
    ...
    def marginal_log_mean_coeff(self, t):
        ...
        """
        log_alpha_t = torch.log(1 - t).to(torch.float64)
        return log_alpha_t
    ...
    def marginal_lambda(self, t):
        ...
        """
        return torch.log(1 / t - 1).to(torch.float64)
...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t):
        ...
        """
        noise, pdf = self.noise_prediction_fn(x, t)
    ...
    def dpm_solver_first_update(self, x, s, t, model_s=None, return_intermediate=False):
        ...
        if self.algorithm_type == "dpmsolver++":
            phi_1 = torch.expm1(-h)
            if model_s is None:
                model_s, pdf_s = self.model_fn(x, s)
            x_t = (
                sigma_t / sigma_s * x
            ...
            if return_intermediate:
                return x_t, {'model_s': model_s, "pdf_s": pdf_s}
            else:
                return x_t
    ...
    def singlestep_dpm_solver_second_update(self, x, s, t, r1=0.5, model_s=None, return_intermediate=False, solver_type='dpmsolver'):
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            model_s1, pdf_s1 = self.model_fn(x_s1, s1)
        ...
        if return_intermediate:
            return x_t, {'model_s': model_s, 'model_s1': model_s1, "pdf_s": pdf_s, "pdf_s1": pdf_s1}
        else:
            return x_t
    ...
    def singlestep_dpm_solver_third_update(self, x, s, t, r1=1./3., r2=2./3., model_s=None, model_s1=None, return_intermediate=False, solver_type='dpmsolver'):
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            model_s2, pdf_s2 = self.model_fn(x_s2, s2)
            if solver_type == 'dpmsolver':
                x_t = (
                    (sigma_t / sigma_s) * x
                    - (alpha_t * phi_1) * model_s
        ...
        if return_intermediate:
            return x_t, {'model_s': model_s, 'model_s1': model_s1, 'model_s2': model_s2, 
                         "pdf_s": pdf_s, "pdf_s1": pdf_s1, "pdf_s2": pdf_s2}
        else:
            return x_t
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        if method == 'adaptive':
            ...
            t = timesteps[step]
            t_prev_list = [t]
            noise, pdf = self.model_fn(x, t)
            model_prev_list = [noise]
            if return_intermediate:
                intermediates_pdf.append(pdf)
            if self.correcting_xt_fn is not None:
            ...
            for step, order in enumerate(orders):
                ...
                if return_intermediate:
                    intermediates.append(x)
                    intermediates_pdf.append(x["pdf_s"])
                    intermediates_pdf.append(x["pdf_s1"])
                    intermediates_pdf.append(x["pdf_s2"])
        else:
            raise ValueError("Got wrong method {}".format(method))
        if denoise_to_zero:
            t = torch.ones((1,)).to(device) * t_0
            x, pdf = self.denoise_to_zero_fn(x, t)
            if self.correcting_xt_fn is not None:
            ...
            if return_intermediate:
                ...
                intermediates_pdf.append(pdf)
        if return_intermediate:
            return x, intermediates, intermediates_pdf
        else:
            return x
