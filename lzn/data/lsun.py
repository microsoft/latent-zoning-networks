...
class LSUN(Data):
    def __init__(self, lsun_root, category, image_size, horizontal_flip=True):
        super().__init__()
        transforms = []
        if horizontal_flip:
            transforms.append(T.RandomHorizontalFlip())
        default_transforms = [T.Resize(image_size), T.ToTensor(), T.Lambda(normalization)]
        self._train_root = os.path.join(lsun_root, f"{category}_train")
        self._test_root = os.path.join(lsun_root, f"{category}_val")
