...
INCEPTION_TFHUB_LOCAL = '/tmp/tfhub_modules/c4be6b55dc0338e82546082d22184ebdc77d76cf'
...
def get_inception_model(inceptionv3=False):
  if inceptionv3:
    ...
    if os.path.exists(INCEPTION_TFHUB_LOCAL):
      print('Loading local inception model')
      model = tfhub.load(INCEPTION_TFHUB_LOCAL)
    else:
      print('Loading remote inception model')
      model = tfhub.load(INCEPTION_TFHUB)
    return model
...
# @tf.function
def run_inception_local(input_tensor,
                        inception_model,
                        num_batches=1,
                        inceptionv3=False):
  """Run the inception network computation locally.
