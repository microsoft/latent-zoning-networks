...
class CleanFID(Metric):
    ...
    def initialize(self, trainer):
        execution_logger.info("Initializing CleanFID metric")
        self._trainer = trainer

        self._clean_fid_feature_extractor = CleanFIDFeatureExtractor()
        self._clean_fid_feature_extractor.to(self._trainer.config.distributed.device)
        self._clean_fid_feature_extractor.eval()
        ...
        for data, label in tqdm(iter(data_loader)):
            ...
            feature = self._clean_fid_feature_extractor(data)
        ...
        execution_logger.info(f"CleanFID labels shape: {labels.shape}")

        num_samples = distributed_reduce_sum(value=num_samples, device=self._trainer.config.distributed.device)
        execution_logger.info(f"CleanFID num_samples: {num_samples}")
        ...
        execution_logger.info(f"CleanFID real features shape: {features.shape}")
    ...
    @torch.no_grad()
    def evaluate(self):
        execution_logger.info("Evaluating CleanFID metric")
        torch.cuda.empty_cache()
        ...
        for i, prior_data in tqdm(enumerate(prior_data_iter)):
            ...
            feature = self._clean_fid_feature_extractor(gen)
            features.append(feature)
            num_samples += gen.shape[0]
            if num_samples >= num_target_samples:
                break
        features = torch.cat(features, dim=0)
        execution_logger.info(f"CleanFID gen features shape local: {features.shape}")
        ...
        execution_logger.info(f"CleanFID gen features shape: {features.shape}")
        features = features.cpu().detach().numpy()
        ...
        metric_item = FloatMetricItem(name="clean_fid", value=fid)
        torch.cuda.empty_cache()
        return [metric_item]
